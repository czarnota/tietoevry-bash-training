...
read -rd '' DOC <<EOF
$SCRIPT_NAME - generate slides, webpage or document from markdown file

USAGE

    $SCRIPT_NAME src.md dst.html           Generate slides dst.html from src.md
    $SCRIPT_NAME --webpage src.md dst.html Generate webpage
    $SCRIPT_NAME --slides src.md dst.html  Generate slides dst.html from src.md

EXAMPLES

Generate slides

    $ $SCRIPT_NAME README.md slides.html

Generate webpage

    $ $SCRIPT_NAME --webpage README.md index.html

OPTIONS

    --webpage       Generate webpage
    --slides        Generate slides (default)
    -h, --help      Print help
EOF

read -rd '' SOURCE_CODE_CSS <<EOF
...
# Base style definitions for presentation format
read -rd '' SLIDES_CSS <<EOF
...
# Javascripts for presentation format
read -rd '' SLIDES_JS <<EOF
/*!	
...
# Html template for presentation format
read -rd '' SLIDES_STYLE <<EOF
<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
...
# Html template for webpage format
read -rd '' PAGE_STYLE <<EOF
<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
...
main () {
    ...
    if [[ ${OPT_slides-1} == 1 ]]; then
        template_contents="$SLIDES_STYLE"
    fi
    if [[ ${OPT_webpage-0} == 1 ]]; then
        template_contents="$PAGE_STYLE"
    fi
